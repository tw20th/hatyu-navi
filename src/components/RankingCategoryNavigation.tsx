...
interface RankingCategoryNavigationProps {
  onCategoryChange: (category: string) => void // ✅ 親に通知する関数
}

export default function RankingCategoryNavigation({
  onCategoryChange,
}: RankingCategoryNavigationProps) {
  const [selectedCategory, setSelectedCategory] = useState('all')

  const handleCategoryChange = (category: string) => {
    setSelectedCategory(category) // ✅ ローカル状態を更新
    onCategoryChange(category) // ✅ 親 (`RankingPage.tsx`) にカテゴリを通知
  }
